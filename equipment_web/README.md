...
| 低庫存耗材預警 | `vw_Admin_Consumable_Alert` |
...
- 查詢使用 View：Web 查詢頁不直接查 Base Tables，讓 View 作為 ANSI-SPARC 外部層，負責欄位遮蔽與角色導向資料呈現。
...
- Flask 使用 Session 模擬登入，MVP 階段不做密碼驗證。
...
1. 在登入頁輸入 `U001`。
2. 進入「資產盤點」，展示財產設備清單來自 `vw_Admin_Asset_Master`。
3. 點選「庫存預警」，展示低庫存耗材來自 `vw_Admin_Consumable_Alert`。
4. 點選「維修工單」，展示全系所有維修工單來自 `vw_Admin_Maintenance_Ticket_Master`。
5. 使用狀態篩選「已完成」，展示管理員可看到已結案工單。
6. 點選「審計軌跡」，展示狀態異動、借用、維修事件來自 `vw_Admin_Audit_Trail`。
7. 登出。
...
7. 點選「可領用耗材」，展示資料來自 `vw_Student_Available_Consumables`。
8. 選擇一筆耗材，填入數量 `1` 與用途 `課程測試`，按下「領用」。
9. 說明：Web 只呼叫 `sp_consume_item`，不直接更新庫存表。
